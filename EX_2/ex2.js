const data = [
  ...
  function moreThen25(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      if (data[i].age > 25) {
      personNames.push(data[i].firstname);
      }
    }
    return personNames;
  }
  ...
  function emailCom(data) {
    let emailEndsWithCom = [];
    for (let i = 0; i < data.length; i++) {
      if (data[i].email.endsWith(".com")) {
      emailEndsWithCom.push(data[i].lastname);
      }
    }
    return emailEndsWithCom;
  }
  ...
  function sexCount(data) {
    ...
    return `Female is: ${femaleCounter} ` + ` Male is: ${maleCounter}`;
  }
